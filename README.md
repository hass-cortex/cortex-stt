...
- **Model-load monitoring** -- a binary sensor per model reflects whether it is currently loaded in GPU/CPU memory
- **Reauthentication** -- a dedicated reauth flow when the server rejects the configured API key
- **Configurable polling** -- polling interval is user-adjustable via the integration's options page
...
Install and start the [Cortex STT Server app](https://github.com/hass-cortex/app-cortex-stt), then download at least one model. Note the server's URL (e.g. `http://homeassistant.local:8769`) and API key.
...
- **Auto-discovery is Supervisor-only** -- Home Assistant Supervisor discovers the app when it is installed on the same host; standalone servers (non-app) must be added manually since the integration does not scan the LAN.
...
- **Stats are per HA session** -- the `Average inference duration` resets on restart because it is computed from session-local counters (the underlying `Total requests` / `Total audio duration` counters are persisted and keep accumulating).
- **No repair issues** -- failures surface as `unavailable` sensors and log lines rather than actionable repair cards.
...
The Cortex STT Server app is built on top of [transcribe-rs](https://github.com/cjpais/transcribe-rs) by [@cjpais](https://github.com/cjpais) -- a unified Rust library providing the multi-engine inference layer (Whisper, Parakeet, SenseVoice, Silero VAD). Many thanks for the excellent foundation.
